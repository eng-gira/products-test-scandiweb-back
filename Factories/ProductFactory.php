...
class ProductFactory
{
    ...
    {
        if ($type==ProductType::Furniture->name) {
            $product = new Furniture();
        } elseif ($type==ProductType::Book->name) {
            $product = new Book();
        } elseif ($type==ProductType::Dvd->name) {
            $product = new Dvd();
        } else {
            throw new \Exception('Failed to initialize Product. Wrong type passed (' . $type .').');
        }
